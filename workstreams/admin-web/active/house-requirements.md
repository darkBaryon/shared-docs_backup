...
1. 有权限员工可以查看全量房源列表。
2. 有权限员工可以查看单个房源详情摘要。
3. 支持按发房方、资产类型、城市区域、房态、发布状态、审核状态筛选。
4. 后台查看不改变房源状态。
...
作为运营员工，我需要查看平台全量房源，以便排查发布状态、审核状态和房态。
...
- 可以按城市、区域筛选。
- 可以按房态、发布状态、审核状态筛选。
- 列表展示房源标题、发房方、城市区域、项目/小区、资产类型、租金、房态、发布状态、审核状态、更新时间。
- 无 `house.view` 权限时不能查看。

### 4.2 查看房源详情

作为运营员工，我需要查看单个房源当前状态，以便判断是否可展示、是否待审核、归属哪个发房方。
...
- `house/list` 和 `house/detail` 需要 `house.view`。
- 第一阶段没有 `house.edit` 接口。
- `house.export` 只保留接口名，不进入实现。
