...
管理后台处于 Phase 1 后端先行阶段。`admin_auth`、`staff`、`role`、`provider` 已完成第一批后端实现，后续模块仍需先补接口级需求和契约再进入代码。

当前不能直接全面开工的原因：

- `house`、`launch_audit` 还缺接口级需求和验收标准。
...
1. `house` 模块实现前补平台房源查询和运营动作边界。
2. 按 `staff` / `role` / `provider` 模块的方式先补 `house` 接口文档，再进入代码实现。
