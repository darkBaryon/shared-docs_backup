...
- handler：`internal/handler/v1/publish/room.(*Handler).UpdateDecentralized`
- service：`internal/service/publish.(*decentralizedRoomService).UpdateDecentralizedRoom`
...
3. 先读 room。
4. 用 `room.decentralized_id` 判断当前房东是否可访问。
5. 通过后更新 HMD。
6. HMD 返回 change。
