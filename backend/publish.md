...
handler/v1/publish
  -> service/publish/PublishService
    ...
    -> repository/hmd
    -> repository/hpd.RootScopeRepository
```

## 当前阶段

- HMD domain service 已拆到 `internal/domain/hmd`。
- HPD 相关 domain 按业务能力拆包：`domain/listingprojection.Service` 负责 HMD -> HPD read model 投影，`domain/publishaccess.Service` 负责发房端 root owner scope。
...
- HPD 已接入 miniapp 与 publisher 两套 projector，后续 outbox worker 复用同一套投影逻辑。
...
- `domain/listingprojection.MiniappProjector`：负责小程序 read model 映射和刷新。
- `domain/listingprojection.PublisherProjector`：负责发房端 `publisher_listing` read model 映射和刷新。
- `domain/publishaccess.Service`：只负责发房端 root owner scope，不写 HMD。
...
- publish 登录主体固定是房东 landlord，不存在 publish staff 登录主线。
- publish 根作用域只依赖 root owner scope relation。
- HMD 不增加 owner/user/staff 字段；`project/community` 的房东归属应由独立 root owner scope relation 表达，其中 `owner_landlord_id` 是主归属键，`owner_phone` 只可作为冗余快照。
...
2. `internal/service/publish`：应用服务 单元测试，验证 HMD 写成功后会调用 HPD `Apply(changes)`，HMD 失败时不调用，读操作不调用，`Apply(changes)` / root scope 失败时会触发 root create 补偿回滚。
