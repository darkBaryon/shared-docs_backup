...
- publish 登录主体固定是房东 user，不存在 publish staff 登录主线。
- publish 根作用域只依赖 root owner scope relation。
- HMD 不增加 owner/user/staff 字段；`project/community` 的房东归属应由独立 root owner scope relation 表达，其中 `owner_user_id` 是主归属键，`owner_phone` 只可作为冗余快照。
